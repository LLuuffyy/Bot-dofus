class dofus.graphics.gapi.ui.Decraft extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   {
      var _loc3_ = null;
      if((_loc3_ = oEvent.target) === this._eaDistantDataProvider)
      {
   ...
   function moveItem(oItem, bAddToWorkshop, nQuantity)
   {
      ...
      if(bAddToWorkshop)
      {
         ...
         else
         {
            if(_loc6_.length >= 50)
            {
               this.api.kernel.showMessage(this.api.lang.getText("INFORMATIONS"),"Vous ne pouvez pas ajouter plus de 50 objets au concasseur.","ERROR_BOX",{name:undefined});
               return undefined;
            }
   ...
   function dropItem(oEvent)
   {
      ...
      else
      {
         ...
         {
            _loc6_ = null;
            if((_loc6_ = oEvent.owner._name) === "_cgGrid")
            {
   ...
   {
      var _loc3_ = null;
      if((_loc3_ = oEvent.params.targetType) === "item")
      {
         if((_loc3_ = oEvent.params.ownerMc._name) === "_cgGrid")
         {
