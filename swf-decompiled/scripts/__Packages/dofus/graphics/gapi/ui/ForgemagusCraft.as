class dofus.graphics.gapi.ui.ForgemagusCraft extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function updateLocalDataVisually()
   {
      ...
      while(_loc6_ < this._eaLocalDataProvider.length)
      {
         _loc7_ = false;
         _loc8_ = 0;
         while(_loc8_ < dofus.graphics.gapi.ui.ForgemagusCraft.ITEMS_ALLOWED_AS_SIGNATURE.length)
         {
            if(dofus.graphics.gapi.ui.ForgemagusCraft.ITEMS_ALLOWED_AS_SIGNATURE[_loc8_] == this._eaLocalDataProvider[_loc6_].unicID)
            {
               _loc5_ = true;
         ...
         _loc9_ = 0;
         while(_loc9_ < dofus.graphics.gapi.ui.ForgemagusCraft.TYPES_ALLOWED_AS_COMPONENT.length)
         {
            if(dofus.graphics.gapi.ui.ForgemagusCraft.TYPES_ALLOWED_AS_COMPONENT[_loc9_] == this._eaLocalDataProvider[_loc6_].type)
            {
               _loc4_ = true;
