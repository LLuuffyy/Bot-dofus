class dofus.graphics.gapi.ui.MapExplorer extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function multipleSelect(aCoords)
   {
      ...
      var _loc11_;
      var _loc12_;
      for(var _loc13_ in aCoords)
      {
         _loc3_ = aCoords[_loc13_];
         if(_loc3_ != undefined)
         {
            _loc4_ = _loc3_.type;
            _loc5_ = _loc3_.x;
            ...
            switch(_loc4_)
            {
               ...
               case 2:
                  ...
                  _loc8_ = _loc5_ + "," + _loc6_ + " (" + this.api.ui.getUIComponent("Party").getMemberById(aCoords[_loc13_].playerID).name + ")";
                  if(_loc8_ != undefined)
                  {
                     break;
                  }
                  delete aCoords[_loc13_];
               case 3:
                  _loc11_ = dofus.Constants.FLAG_MAP_SEEK;
                  _loc8_ = _loc5_ + "," + _loc6_ + " (" + aCoords[_loc13_].playerName + ")";
                  break;
               default:
                  _loc11_ = dofus.Constants.FLAG_MAP_OTHERS;
            }
            _loc12_ = this._mnMap.addXtraClip("UI_MapExplorerFlag","highlight","highlight" + _loc4_ + _loc13_,_loc5_,_loc6_,_loc11_,100,false,true);
            if(_loc8_ != undefined)
            {
   ...
   function initData()
   {
      ...
      this.setupTravelContextMenu();
      if(this.api.datacenter.Basics.autopilotPath != undefined && this.api.datacenter.Basics.autopilotPath.length > 0)
      {
         this.addToQueue({object:this,method:this.updateAutopilotPath,params:[this.api.datacenter.Basics.autopilotPath]});
      }
   }
   ...
   function initDungeonMap()
   {
      ...
      for(var _loc7_ in _loc3_)
      {
         ...
         {
            _loc6_ = this._mnMap.addXtraClip("UI_MapExplorerRectangle","dungeonMap","dungeonCell" + _loc7_,_loc5_.x,_loc5_.y);
   ...
   function out(oEvent)
   {
      var _loc3_;
      var _loc4_;
      if((_loc3_ = oEvent.target) !== this._mnMap)
      {
         this.gapi.hideTooltip();
      }
      else
      {
         _loc4_ = 0;
         while(_loc4_ < dofus.graphics.gapi.ui.MapExplorer.DIRECTIONS.length)
         {
            this.setMovieClipTransform(this["_mcTriangle" + dofus.graphics.gapi.ui.MapExplorer.DIRECTIONS[_loc4_]],dofus.graphics.gapi.ui.MapExplorer.OUT_TRIANGLE_TRANSFORM);
            _loc4_ += 1;
         }
      }
   }
   ...
   function select(oEvent)
   {
      ...
      var _loc4_ = oEvent.coordinates;
      this._mnMap.clear("flag");
      if(this.api.kernel.GameManager.updateCompass(_loc4_.x,_loc4_.y,false))
      {
         this._mnMap.addXtraClip("UI_MapExplorerFlag","flag","targetCoords",_loc4_.x,_loc4_.y,255,100,false,true);
      }
      var _loc5_;
      var _loc6_;
      var _loc7_;
      ...
      if(!this.api.datacenter.Game.isFight && !this.isDungeonMapDisplayed)
      {
         ...
         if(_loc5_ != undefined)
         {
            _loc6_ = this.api.kernel.AreasManager.getSubAreaIDFromCoordinates(_loc4_.x,_loc4_.y,this._dmMap.superarea);
            _loc7_ = "";
            if(_loc6_ != undefined)
            {
               _loc8_ = this.api.lang.getMapSubAreaText(_loc6_).n;
            ...
            _loc5_.addStaticItem(_loc7_);
            _loc5_.addItem("Voyager ici",this,this.onTravelClick,[_loc4_.x,_loc4_.y],true);
   ...
   function setupTravelContextMenu()
   {
      var mapExplorer = this;
      var _loc2_ = new ContextMenu(function()
      {
         if(mapExplorer._mnMap._ldrMap != undefined)
         {
            mapExplorer._nRightClickMapX = mapExplorer._mnMap._ldrMap._xmouse;
            mapExplorer._nRightClickMapY = mapExplorer._mnMap._ldrMap._ymouse;
         }
      });
      _loc2_.hideBuiltInItems();
      var _loc3_ = new ContextMenuItem("Voyager ici",function()
      {
         var _loc1_;
         if(!mapExplorer.api.datacenter.Game.isFight && !mapExplorer.isDungeonMapDisplayed)
         {
            _loc1_ = mapExplorer._mnMap.getCoordinatesFromReal(mapExplorer._nRightClickMapX,mapExplorer._nRightClickMapY);
            if(_loc1_.x != undefined && _loc1_.y != undefined)
            {
               mapExplorer.api.network.Basics.travelCommand(_loc1_.x,_loc1_.y);
            }
         }
      });
      _loc2_.customItems.push(_loc3_);
      this._mnMap.menu = _loc2_;
   }
   function updateAutopilotPath(aPath)
   {
      var _loc3_ = ["ACTUEL(vert)","A_FAIRE(cyan)","ZAAP(jaune)","DEST(rouge)","FAIT(gris)"];
      this._mnMap.clear("autopilotPath");
      if(aPath == undefined || aPath.length <= 0)
      {
         return undefined;
      }
      this._mnMap.createXtraLayer("autopilotPath");
      var _loc4_ = 0;
      var _loc5_;
      var _loc6_;
      var _loc7_;
      while(_loc4_ < aPath.length)
      {
         _loc5_ = aPath[_loc4_];
         switch(_loc5_.type)
         {
         ...
         _loc7_ = this._mnMap.addXtraClip("UI_MapExplorerSelectRectangle","autopilotPath","autopilot" + _loc4_,_loc5_.x,_loc5_.y,_loc6_,60);
         _loc4_ += 1;
      }
   }
}
