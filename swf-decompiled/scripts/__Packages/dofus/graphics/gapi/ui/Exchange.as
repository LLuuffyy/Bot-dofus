class dofus.graphics.gapi.ui.Exchange extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function initTexts()
   {
      ...
      this._lblOgrine.text = new ank.utils.ExtendedString(this.api.datacenter.Player.Ogrine).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3);
   }
   function validateOgrine(nQuantity)
   {
      if(nQuantity > this.api.datacenter.Player.Ogrine)
      {
         nQuantity = this.api.datacenter.Player.Ogrine;
      }
      this.api.network.Exchange.movementOgrine(nQuantity);
   }
   function initData()
   {
   ...
   function validate(oEvent)
   {
      switch(oEvent.params.targetType)
      {
         ...
         case "kama":
            ...
            return undefined;
         case "ogrine":
            this.validateOgrine(oEvent.value);
            return undefined;
         default:
            return undefined;
      }
   }
   function localOgrineChange(oEvent)
   {
      this._lblLocalOgrine.text = new ank.utils.ExtendedString(oEvent.value).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3);
      this.hideButtonValidate(true);
      ...
      this._lblOgrine.text = new ank.utils.ExtendedString(this.api.datacenter.Player.Ogrine - oEvent.value).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3);
      this.onDataUpdated();
   }
   function distantOgrineChange(oEvent)
   {
      this._mcBlink.play();
      this._lblDistantOgrine.text = new ank.utils.ExtendedString(oEvent.value).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3);
      this.hideButtonValidate(true);
      this.onDataUpdated();
   }
   function playerOgrineChange(oEvent)
   {
      this._lblOgrine.text = new ank.utils.ExtendedString(oEvent.value).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3);
   }
   ...
   function over(oEvent)
   {
      switch(oEvent.target)
      {
         ...
         case this._lblOgrine:
            this.api.ui.showTooltip(this.api.datacenter.Player.Ogrine,oEvent.target,-20);
            return undefined;
         default:
            return undefined;
      }
